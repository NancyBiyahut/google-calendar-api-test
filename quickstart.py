...
def commitHours(creds):
    try:
        ...
        today_ist = datetime.datetime.now(ist_timezone).date()

        # Construct start and end times in IST
        timeStart = datetime.datetime.combine(today_ist, datetime.time.min).astimezone(pytz.utc)
        timeEnd = datetime.datetime.combine(today_ist, datetime.time.max).astimezone(pytz.utc)

        # Format the times in ISO 8601 format
        timeStart_formatted = timeStart.isoformat()
        timeEnd_formatted = timeEnd.isoformat()
        print("Getting today's coding hours")
        events_result = service.events().list(calendarId='1138f1bddfe6076b55d204ff5a9aa09dd3e71f23ca54e893a9d0f80147489098@group.calendar.google.com', timeMin=timeStart_formatted,
                                              timeMax = timeEnd_formatted,maxResults=10, singleEvents=True,
...
def addEvent(creds, duration, description):
    ...
    end = datetime.datetime.now(ist_timezone) + datetime.timedelta(hours=int(duration))
    ...
    service = build('calendar', 'v3', credentials=creds)
    event = service.events().insert(calendarId= '1138f1bddfe6076b55d204ff5a9aa09dd3e71f23ca54e893a9d0f80147489098@group.calendar.google.com', body=event).execute()
